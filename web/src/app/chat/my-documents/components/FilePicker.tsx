                ...
                <LocalFilesDropdown
                  onFileSelect={async (file) => {
                    try {
                      ...
                      // Call MCP server to add local file
                      ...
                      if (data.success) {
                        setPopup({
                          message: `Successfully ingested ${file.name} into Onyx (${data.chunks} chunks processed)`,
                          type: "success",
                        });
                        // Refresh the folders to show the new file
                        await refreshFolders();
                      } else {
                  ...
                  onFolderSelect={async (folder) => {
                    try {
                      console.log("Selected local folder:", folder);
                      
                      // Call MCP server to add local folder
                      const response = await fetch('/api/mcp/local-files', {
                        method: 'POST',
                        headers: {
                          'Content-Type': 'application/json',
                      ...
                      if (data.success) {
                        setPopup({
                          message: `Successfully ingested ${folder.name} folder into Onyx (${data.chunks} chunks processed)`,
                          type: "success",
                        });
                        // Refresh the folders to show the new folder
                        await refreshFolders();
                      } else {
                        throw new Error(data.error);
                      }
                      ...
                      setPopup({
                        message: `Failed to add ${folder.name}: ${error instanceof Error ? error.message : 'Unknown error'}`,
                        type: "error",
                      });
                    }
                  }}
